...
public abstract class HttpServerTracer<REQUEST, RESPONSE, CONNECTION, STORAGE> extends BaseTracer {
	...
	public Context startSpan(REQUEST request, CONNECTION connection, String spanName, long startTimestamp) {
		...
		Span.Builder  builder = tracer.spanBuilder(spanName).setSpanKind(SERVER).setParent(parentContext);
	...
	public Context startSpan(REQUEST request, CONNECTION connection, String spanName, Context parentContext) {
		return startSpan(request, connection, spanName, parentContext, -1);
	}

	public Context startSpan(REQUEST request, CONNECTION connection, String spanName, Context context, long startTimestamp) {
		Context parentContext = extract(request, getGetter());
		Span.Builder builder;
		if(Context.root() == parentContext){
		builder = tracer.spanBuilder(spanName).setSpanKind(SERVER).setParent(context);
		parentContext = context;
		}else{
			builder = tracer.spanBuilder(spanName).setSpanKind(SERVER).setParent(parentContext);
		}
		if (startTimestamp >= 0) {
			builder.setStartTimestamp(startTimestamp);
		}
	...
	/**
	 * Creates new scoped context, based on the current context, with the given
	 * span.
	 *
	 * <p>
	 * Attaches new context to the request to avoid creating duplicate server
	 * spans.
	 */
	...
	/**
	 * Creates new scoped context, based on the given context, with the given
	 * span.
	 *
	 * <p>
	 * Attaches new context to the request to avoid creating duplicate server
	...
	// Changes
	// This Method was Newly Added For Custom Tracing
	public Context startContext(Span span, STORAGE storage, Context context) {
		// TODO we could do this in one go, but
		// TracingContextUtils.CONTEXT_SPAN_KEY is private
		Span parentSpan = Span.fromContext(Context.current());
		Context newContext = context.with(CONTEXT_SERVER_SPAN_KEY, span).with(CONTEXT_CLIENT_SPAN_KEY, parentSpan).with(span);
		attachServerContext(newContext, storage);
		return newContext;
	}

	/**
	 * Convenience method. Delegates to {@link #end(Span, Object, long)},
	 ...
	 * {@link #endExceptionally(Span, Throwable, Object)}, passing
	...
	/**
	 * Convenience method. Delegates to
	 * {@link #endExceptionally(Span, Throwable, Object, long)}, passing
	 * {@code timestamp} value of {@code -1}.
	 */
	...
	/**
	 * If {@code response} is {@code null}, the {@code http.status_code} will be
	 * set to {@code 500} and the {@link Span} status will be set to
	 * {@link io.opentelemetry.api.trace.StatusCode#ERROR}.
	 */
	 ...
	 * {@link #attachServerContext(Context, Object)}.
	 */
	@Nullable
	...
	protected void onConnection(Span span, CONNECTION connection) {
		...
		// Negative or Zero ports might represent an unset/null value for an int
		// type. Skip setting.
		if (port != null && port > 0) {
	...
	/*
	 * https://github.com/open-telemetry/opentelemetry-specification/blob/master
	 * /specification/trace/semantic_conventions/http.md
	 * 
	 * HTTP semantic convention recommends setting http.scheme, http.host,
	 * http.target attributes instead of http.url because it "is usually not
	 * readily available on the server side but would have to be assembled in a
	 * cumbersome and sometimes lossy process from other information".
	 * 
	 * But in Java world there is no standard way to access
	 * "The full request target as passed in a HTTP request line or equivalent"
	 * which is the recommended value for http.target attribute. Therefore we
	 * cannot use any of the recommended combinations of attributes and are
	 * forced to use http.url.
	 */
	...
	static String extractForwardedFor(String forwarded) {
		...
		if (start >= forwarded.length() - 1) { // the value after for= must not
												// be empty
			return null;
		}
	...
	private <C> Context extract(C carrier, TextMapPropagator.Getter<C> getter) {
		...
		// Using Context.ROOT here may be quite unexpected, but the reason is
		// simple.
		// We want either span context extracted from the carrier or invalid
		// one.
		// We DO NOT want any span context potentially lingering in the current
		// context.
	...
	private static void setStatus(Span span, int status) {
		...
		// TODO status_message
		// See
		// https://github.com/open-telemetry/opentelemetry-specification/issues/950
	...
	/**
	 * Stores given context in the given request-response-loop storage in
	 * implementation specific way.
	 */
	protected abstract void attachServerContext(Context context, STORAGE storage);

	/*
	 * We are making quite simple check by just verifying the presence of
	 * schema.
	 */
